...
def convert_depth_back_from_m1_255(tensor):
    return (tensor + 1.0) * 127.5


def convert_depth_back_from_255_up(tensor, low_thres, up_thres):
    return tensor * up_thres / low_thres
...
def perse2ortho(depth_p, color_p, intrinsics_p, intrinsics_o):
    ...
    x_cord_p = (X1 - cx) * inv_fx * depth               ##f/h1 = Z/h2
...
# requires_grad: 固定一些层，不让该网络进行梯度计算
